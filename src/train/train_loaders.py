...
def train_model_with_dataloaders(epochs=1, model=None, criterion=None, optimizer=None, 
    ...
    for epoch in tqdm(range(epochs), desc="Training Progress", unit="epoch"):
        ...
        with tqdm(total=len(train_loader), desc=f"Epoch {epoch + 1}/{epochs} - Training", position=1, leave=False) as pbar_train:
        ...
        with torch.no_grad():
            ...
            with tqdm(total=len(val_loader), desc=f"Epoch {epoch + 1}/{epochs} - Validation", position=2, leave=False) as pbar_val:
